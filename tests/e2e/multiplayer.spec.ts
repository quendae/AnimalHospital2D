...
test.describe.serial("P2P co-op", () => {
  test("host and guest form a P2P room, start together and sync guest movement", async ({ browser }) => {
    ...
    try {
      await host.page.getByTestId("host-room").click();
      await expect(host.page.getByTestId("room-panel")).toBeVisible();
      const roomCode = (await host.page.getByTestId("room-code").textContent())?.trim();
      ...
      const remoteXBefore = await host.page.evaluate(() => {
        ...
        return actor.sprite.x as number;
      });

      await guest.page.keyboard.down("d");
      await guest.page.waitForTimeout(650);
      ...
      await expect.poll(async () => host.page.evaluate(() => {
        ...
        const actor = [...scene.__p2pBridgeRuntime.actors.values()][0] as any;
        return actor.sprite.x as number;
      }), { timeout: 5_000 }).toBeGreaterThan(remoteXBefore + 20);
